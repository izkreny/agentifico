...
**The ban's first reason holds here and does not decide it.** Every commit on a branch does "repeat the same value", and on an unstacked branch - which is the ordinary shape, since *Work outside an epic is not stacked* in `AGENTS.md` reserves stacking for an epic - it repeats with nothing to distinguish it from. What a repeating value costs is nothing: it is decided once, by the issue's package label, so no commit spends a judgement on it. What it buys is a commit that says which of this repository's packages it belongs to without its branch name beside it, which is how it is read in `git log --all`, in a review queue holding several packages' branches at once, and in a `git bisect` that crosses branches. A stack is not where it earns that: an epic sits inside one package, so the scope repeats across the whole stack too, and what tells those branches apart is the `(#{issue-number})` the header already carries.
