...
**A package epic's last child is that package's sweep issue, and its branch is the stack's top branch.** The epic is what closes, so a sweep issue sitting outside it is a precondition nothing enforces: the epic's progress never counts it, every child can merge with the sweep unopened, and the tag then stands on a package nobody read whole. Making it the last child is what puts it on the top branch. The rule assumes one package per epic, which is what this repository has; an epic spanning two would owe a sweep child and a top branch for each, and one stack has one top, so such an epic is where that rule gets written rather than inferred from this one.
...
**A package is tagged only after its own whole-package sweep has run.** What a tag asserts is that the package was read whole rather than diff by diff, and no branch's review can supply that: a review reads a diff, so a file no branch has touched since it was written is never read whole by anyone. The sweep is `/skills-maker review` over the package's own directory, which for a plugin covers the skills under its `skills/` and the files that belong to no skill - the manifest, the agents, the hooks and the README - which nothing else covers. An open sweep issue asserts nothing; `.agents/gh-solo.md` is where that requirement is enforced.
