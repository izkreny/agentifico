...
**Nothing is deferred: every finding is fixed on this branch.** The plan first proposed deferring five findings whose fix is a file split rather than a wording change - `PF-6`, `PF-7`, `TR-13` and the structural halves of `TR-7` and `RV-5` - on the grounds that a split changes what every installed user loads and cannot be reviewed as a wording diff. The owner overruled that: the sweep's fourth criterion is satisfied by fixing, and a deferral leaves the tag standing on a package whose known defects are still in it. So the splits land here too, and the branch is larger for it.

**The review at the end is a full round with no scoped re-review.** The owner's call, recorded because it changes what the branch's own verification is worth: the round reads the whole diff rather than re-reading fixes against earlier findings.
