...
- **Did the ask on this plan's `## Open questions` mean a documentation gap or a real fallback?** Answered as the first, since the second is the optional design the same comment ruled out. Asked in the thread; the plan implements the documentation reading.

## Settled

- **Does `head` become required, moving the major, or optional with a recorded fallback, moving the minor?** Required, and `5.0.0`. An optional field with a fallback leaves the defect live on every round that omits it, and a check something can quietly skip is one that will be skipped.
