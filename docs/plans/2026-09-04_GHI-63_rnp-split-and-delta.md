...
**The span is a link, not a string.** It points at the blob for that path at the head the push landed, with a `#L{start}-L{end}` fragment, so a row is one click from the lines it names instead of a coordinate the reader has to go and find. The sha rather than a branch name, because a branch link would drift the moment the next commit lands; the cost is that a later `gh stack sync` rewrites that sha and the link goes stale, which is correct for a record of one push read at that push.

**No gate reads it, and each of the three was checked rather than assumed.** `plugins/gh-solo/skills/pr-flow/workflows/merge.md`'s thread gate reads `reviewThreads`, and a Conversation comment is not one. `plugins/gh-solo/skills/pr-flow/scripts/post-review.py`'s `highest-id` counts `::RF{n}::` and the legacy line-start form only, so the bare `RF4` in a row is ignored - and its warning about an ignored bare id fires only for one *above* the answer, which an already-issued id never is. #16's pass count reads the reviews surface, where this comment does not appear at all.
...
- State in that step the row's shape - the `path:start-end` span as a blob link at the pushed head, then the `RF{n}` or `-` - that the row count is uncapped as a record row, that it opens no thread and resolves nothing, and that it assigns no id.
