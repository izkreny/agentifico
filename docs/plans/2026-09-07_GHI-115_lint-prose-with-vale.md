...
**The phrase lists are mined from the whole repository, not invented, and the exceptions come from the same dig.** Before any rule file is written, subagents read every surface where a phrase was ever caught or defended, and every item comes back with its source. The surfaces, each read to its end rather than sampled:

- **Every review thread on every pull request**, open, merged or closed, paginated to the last comment. An `RF` finding or an owner comment that names a phrase is a token; an owner refusal of such a finding is an exception or a guards line, with the thread as its reason. Review summary bodies and Conversation comments are read in the same pass.
- **Every commit**, on the trunk and on every branch still on the remote: its message, since a subject and body are prose that reviews have corrected too, and its diff, for the phrase removed and the phrase that replaced it. Squash commits carry the pull request bodies, whose `## Settled` sections record decisions about wording.
- **Every issue body and comment**, above all the sweep issues, whose bodies list what a whole-package read found.
- **The prose itself as a corpus**: the examples `skills/skills-maker/workflows/new.md`, `skills/skills-maker/workflows/review.md` and `AGENTS.md` give in their own text, then the draft rules run over every markdown file in the repository, `plugins/gh-solo`, every package under `skills/`, `docs/plans`, `AGENTS.md` and `.agents/gh-solo.md`, with each hit triaged as a token calibration, a guards line or a true finding. Only `skills/skills-maker` gets its true findings fixed on this branch; the rest go to the handoff.

The dig reports its coverage in numbers: pull requests read, threads read, commits diffed, issues read, files linted, so a pass that stopped early is visible rather than assumed. Each token in a rule file carries a YAML comment naming the finding, thread or commit it came from, and each exception carries the thread that defended it. A token with no source is not added, and a phrase the dig returns that no rule covers is reported in the handoff rather than given a rule with no `skills/skills-maker/workflows/new.md` heading behind it.
...
- Dig the whole repository for tokens and exceptions: every review thread, review body and Conversation comment on every PR, every commit on every remote branch with its message and diff, every issue, and the prose corpus itself under the draft rules; report the coverage counts and return each phrase with its source.
