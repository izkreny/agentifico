...
- Watch every fixture fail against the current reader before trusting it. The table above says which shape each of the style fixtures catches; the duplicate-key one fails against the current code for its own reason, and the two the current code already satisfies - the parse-error fallback and the plain-scalar comment - are watched against a deliberately broken version of the thing they test.
