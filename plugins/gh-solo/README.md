...
The plugin installs a single hook. A `git push` it can see writing to the trunk asks for confirmation first, and says why. It reads the command the way a shell would, through the quoting, through `&&` and newlines, and into a nested `bash -c` or `eval`; what it cannot see is a destination that only exists at runtime, and it fails open rather than guessing. Every skill here states that work reaches the trunk only through a reviewed pull request's squash merge, and a rule written in prose is a request rather than a stop.
