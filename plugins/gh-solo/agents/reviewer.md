...
**`effort` is pinned and `model` is not, and that is deliberate rather than an oversight to tidy away.** Pinning effort decouples a review's depth from whatever the orchestrating session happened to be set to, which is the author's session; leaving the model inherited keeps this plugin from overriding the model every consumer pays for, and a repository that wants otherwise says so with a `Reviewer model:` line in its own config. **The effort line is not a guarantee, and a model asked for at spawn time is.** The documentation has `CLAUDE_CODE_EFFORT_LEVEL` outrank the `effort` declared here, and has a per-spawn model outrank `CLAUDE_CODE_SUBAGENT_MODEL`, which is a default rather than an override unless `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` is set. So a round reports the model it asked for as the model that ran, and its effort as a request.
