...
**On the `rescope` entrance the prompt carries exactly three further things, and they are sanctioned rather than contamination**: the commit range the fixes landed in, the findings you are answering about with their ids, and which commit claims which id. Each is an address rather than an account - where to look, what to look for, and which claim belongs to which commit - and the pass cannot be done without them, since answering "did this fix close that finding" requires knowing which fix and which finding. A prompt carrying anything *beyond* those three is the contamination the paragraph above describes, and the same report rule applies to it.
