...
Each is an address rather than an account - where to look, what to look for, and which claim belongs to which commit, the same test the full entrance's head sha passes - and the pass cannot be done without them, since answering "did this fix close that finding" requires knowing which fix and which finding. A prompt carrying anything *beyond* those three is the contamination the full entrance's rule describes, and the same report rule applies to it.
