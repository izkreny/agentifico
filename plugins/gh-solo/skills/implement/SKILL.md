...
- **The floor under every repository:** a behavior change carries a test, the repo's stated gates are the gates that run, and a branch whose plan names no gates in `## Verification` is refused rather than implemented, because shipping unverified code is the alternative.
