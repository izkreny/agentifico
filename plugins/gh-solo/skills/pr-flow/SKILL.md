...
**The domain is the `via` line itself**, rather than a list of surfaces, so it cannot drift from the enumeration in the `via` bullet above and it reaches a surface nobody has written yet. The PR body sits outside it, for the same reason it carries no `via` line, and so does **anything `scripts/post-review.py` composes** - a finding and a record Review are built from the reviewer's findings file rather than written here, and `../reviewer/references/baseline.md` drops an inherited word cap on a finding deliberately, for a reason that is sound there and does not extend to conversation.

**Within a post that is in the domain, this is not counted**, because none of it is prose whose length the agent chose:
