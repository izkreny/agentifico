...
- **Console output is markdown, in every workflow here.** **A printed table has its cells padded so the columns line up**, header rule included: a terminal that does not render markdown turns an unpadded table into a wall of `|` and `-`, and the alignment is the whole reason a table beats a list there.
...
**`references/post-caps.md` owns the cap and is its authority**: how long a post carrying a `via` line may be, what the domain is, what falls outside it entirely, what sits inside a capped post without counting, and what happens where the owner's global instructions file sets its own. It is not summarised here, because a summary of a number is a second home for the number. Read it before composing anything that carries a `via` line.
...
| `rnp`, "resolve all and push", "we are done", "you can merge" - said after a review round left fix commits unpushed | `workflows/resolve.md`, the protocol's step 7: stop the watch, post the authorisation, resolve what it covers, push, release what was held for the push, read the checks, then `workflows/merge.md`                 |
...
- **`scripts/post-review.py`** - builds the one API call that lands a round's threads and its record Review together, refusing the whole round on any invalid finding; holds a finding whose line only the unpushed fixes carry and releases it into a thread once they are pushed; afterwards reconciles what the PR carries against what was sent, and reads the highest `RF{n}` already on it, which the round needs before it can assign the next; `scripts/test-post-review.sh` is its bench
...
- **`workflows/resolve.md`** - the protocol's step 7: the authorisation comment and its marker line, the resolve mutation, the round's only push, and the release that turns a held finding into its thread
