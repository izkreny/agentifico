...
- **Repository-specific conventions live in the repository**, in its agent config file (`.agents/github.md`, or `.claude/github.md` where that is what the repo uses). Read it when present and let it override the defaults here. Nothing repo-specific belongs in this skill. **Which reviewer a round spawns is one of those facts:** a `Reviewer agent:` line there names an agent type to use instead of the bundled one, and `workflows/review.md` Step 1 owns what that agent must return and why an unregistered name is a refusal rather than a fallback.
