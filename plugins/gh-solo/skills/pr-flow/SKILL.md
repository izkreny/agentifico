...
- **Repository-specific conventions live in the repository**, in its agent config file (`.agents/gh-solo.md`, or `.claude/gh-solo.md` where that is what the repo uses). Read it when present and let it override the defaults here. Nothing repo-specific belongs in this skill. **Which reviewer a round uses is one of those facts:** a `Reviewer agent:` line there names an agent type to spawn instead of the bundled one, a `Reviewer command:` line names a capability to invoke instead, whose findings come back to the orchestrator and go up through the same posting script, and a `Reviewer model:` line names the model the round asks the spawn for. `workflows/review.md` Step 1 owns all three forms, what each must produce, why an unregistered agent is a refusal rather than a fallback, and why the model key does not reach the invoked-command form.
