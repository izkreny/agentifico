...
- **A new defect anchored to the fixes themselves gets no thread this round, and that is a property of the API rather than a choice.** The fixes are unpushed at step 5, so a line only they carry is not in the pull request's diff and GitHub cannot resolve an anchor to it; the posting call is atomic, so attempting it would take the whole record down, verdicts for the closed findings included. It keeps its `RF{n}`, travels in the record Review's body and the round report with its `file:line`, and its thread opens after step 7's push. **The round report must say which findings got a thread and which could not**, or a reader takes the second for an absence of findings.
