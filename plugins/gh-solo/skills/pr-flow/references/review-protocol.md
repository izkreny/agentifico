...
- **A new defect anchored to the fixes themselves gets no thread and no `RF{n}`, and that is a property of the API rather than a choice.** The fixes are unpushed at step 5, so a line only they carry is not in the pull request's diff and GitHub cannot resolve an anchor to it; the posting call is atomic, so attempting it would take the whole record down, verdicts for the closed findings included. An id would be no better than no id: nothing this flow can post would carry it, so the next round's highest-id read would not see it and would reissue it, breaking *Ids never restart*. It goes to the owner in the round report with its `file:line`, and after step 7's push the line is ordinary, so the next full pass anchors it without special handling. **The round report must say which findings got a thread and which could not**, or a reader takes the second for an absence of findings. Carrying such a finding properly needs the posting script to grow a route for it. `workflows/review.md` owns that limitation and names the issue tracking it; when that lands, this paragraph is what changes.
