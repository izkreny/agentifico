...
- **An id is *issued* as `::RF{n}::` and named in prose as anything else.** The delimiter is what the highest-id read counts, so an explanation free to write `RF7` as an example cannot raise the counter, and no writer has to remember a convention. A bare id opening a line is read too, for the life of every pull request posted before the delimiter existed.
