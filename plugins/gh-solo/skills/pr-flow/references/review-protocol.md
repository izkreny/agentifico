...
- **An id named as an example in prose is written non-literally.** The highest-id read scans every `RF{n}` in every comment body rather than only the position `finding_body` writes it at, and it has to: a held id lives in a ledger row and in that ledger's JSON, neither of which sits there. So a post saying `RF7` to illustrate the mechanism raises the counter, and the next round's findings start above it. Write `RF{n}`, or name the finding in words. The read stays permissive rather than being narrowed to fix this, because a gap in the sequence costs nothing while a reused id breaks the rule above for the life of the pull request.
- **A finding comment opens with its id at the start of the line under the header**, which is `finding_body`'s shape and which `release` depends on to tell a posted finding from a post that mentions one. An edit that moves the id off the line start makes a held finding look already threaded and drops it silently.
