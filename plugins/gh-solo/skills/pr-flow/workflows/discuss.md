...
- **A question is not a decision.** The owner's vocabulary in `references/review-protocol.md` reads a reply of theirs as settling a finding, and `workflows/resolve.md` acts on it; that rule holds only for replies that actually settle something. Misreading a question as a verdict leaves it unanswered forever.
