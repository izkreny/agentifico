...
**What does not count is *Never counted* under *Post caps* in `SKILL.md`**, which reaches a body section unchanged: a fence, a table, the owner's own words quoted, and a record row - which is what the `## Steps` and `## Verification` checkbox lists are, their length set by how many steps and gates a branch has rather than by how much was written. Those two lists are uncapped, and that is why neither is named above.
...
**This section owns the whole route, and every other site points here** rather than restating a leg of it, because a route stated in several places is one that gets half-fixed. It runs in three parts:
...
- **The body's own edit lands with that commit's push**, never before it, so the body never names a plan entry that exists on one machine. Which push that is depends on when the decision settled: **before implementation** it is the pre-spawn sync in the `implement` skill, which writes the plan file, pushes it and edits the body in one pass; **after implementation** it is the review protocol's step 7, the round's only push, per `workflows/resolve.md`. Every round's commits reach one of the two, so no decision is left with nowhere to go.
- **The section may exceed its cap while its relocation commit is held**, and only then. That window is the sanctioned one, it closes at the push, and the **Body capped** row in `workflows/review.md` reads it as compliant rather than as a breach.
