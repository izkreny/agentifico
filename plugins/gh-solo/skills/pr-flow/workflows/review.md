...
| **Body capped** | Every section *Body caps* in `workflows/open.md` names is within the cap it sets, applying the exclusions it points at and its `## Settled` denial. Count them; mechanical, not a judgement. `## Plan overview` also links the plan file rather than naming it in backticks. **Unlike the post rows below there is no latency**: the body exists before the round starts, so a breach is caught in the round that reads it |
