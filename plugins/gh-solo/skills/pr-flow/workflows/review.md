...
4. **Check the conventions**, per *Convention checks* at the end of this file, and post the failures as one Review:
   ...
   `COMMENT`, not `REQUEST_CHANGES`: a missing assignee is a one-command fix, not a reason to mark a PR as blocked. **Not the `pulls/<pr-number>/comments` endpoint**, which anchors to a file and line: every convention finding is a fact about the PR as a whole and has nowhere in the code to anchor to. Disclaimer and `via` line first per `SKILL.md`, the latter reading: via `pr-flow` review, convention check; its length is the post-cap bullet's, in the same file, and the failure list is a record row rather than prose, so a long list of failures is not a breach.
...
7. **Post the reviewer's report as a Conversation comment**, `gh pr comment <pr-number> --body-file <scratch>`, disclaimer and `via` line first: via `pr-flow` review, round report. The reviewer's report text goes below it unchanged, and is relayed verbatim so the post-cap bullet in `SKILL.md` does not count it - what that cap bounds here is whatever you write around it, and its companion rule forbids re-listing findings that are already threads.
...
| **Posts capped** | Every post on the PR carrying a `via` line is within the post-cap bullet in `SKILL.md` and does not restate what the reader is already looking at. Count them, applying that bullet's exclusions; mechanical, not a judgement. **This audits the previous round, never this one** - a round cannot check posts it has not made yet, so a breach surfaces one round late |
