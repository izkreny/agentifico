...
**The line must carry `{sha}`, and a line without one refuses the round:** `⛔ REFUSED - the Reviewer command: line carries no {sha}, so the pass cannot be pinned`. A capability that takes no revision reads whatever the pull request holds at the moment it runs, so a push in that window makes it review one version while the round compares another and refuses - discarding a pass whose every anchor was valid against what it actually read, which is the failure this whole mechanism exists to remove rather than relocate. Refusing at the config read is what keeps that failure out: it costs a one-line edit to a file the owner controls, where the alternative costs a reviewer pass and reports the loss as something the reviewer did.
...
**This refusal is prose rather than a gate, and that is a limit worth naming.** `scripts/post-review.py` never sees `.agents/gh-solo.md`, so no bench can catch a `Reviewer command:` line that lost its `{sha}`; what stands in its place is that the substitution and the refusal are stated together, here, at the one point the line is read.
