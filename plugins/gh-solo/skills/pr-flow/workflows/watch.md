...
- **Every emitted record is deduplicated on its `id` plus its own timestamp**, in the script's `seen` set. The window overlaps by a second at every boundary: `since` is stamped to whole seconds, the REST call admits a record whose `updated_at` equals it, and the client-side comparisons treat two strings equal to the second as not-greater. So anything written inside the second that `since` names is delivered in the cycle that finds it and again in the next one. Observed twice on the same PR, payloads byte-identical 32 seconds apart: a four-comment review at 20:43:21 and 20:43:53, a review body at 08:31:22 and 08:31:54. A strict comparison would trade the duplicate for a silent drop - a comment written after a cycle's request but inside the second it stamped would then never match again - so the overlap stays and `seen` removes the repeat. The timestamp in the key is what lets a genuinely edited comment through a second time. This is not cosmetic: every emitted line is an order or a question, and `OK, fix it then` delivered twice invites the fix twice.
