...
- **An anchor**: `path`, `line` and `side`.
  ...
  - **An unanchored finding cannot be posted**, so a finding you cannot anchor is one you must either anchor by reading the diff more carefully or drop. There is no such thing as a finding about the pull request in general. **The `rescope` pass is where that rule is qualified, and `workflows/rescope.md` owns the qualification**: a line only the unpushed fix commits carry cannot be anchored yet and goes into the file regardless, because what can be anchored now is decided after you, and a finding you leave out is lost rather than deferred.
