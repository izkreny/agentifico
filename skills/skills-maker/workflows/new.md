...
Write it to a file and pass the path - `--body-file <file>`, `-F body=@<file>` - because a line break marks the payload as prose, and prose carries the `'`, the backtick and the `#` that shell quoting mangles: the `'` ends a single-quoted string, the backtick executes inside a double-quoted one, and the `#` opens a comment wherever the quoting slips, so the payload lands mangled, as posted, under the owner's name. The test is the payload's own text in the skill rather than where it is going: a one-line title, a label or a date gains nothing from a file, and a quoted span that breaks a line or carries either character is over the line whatever it is bound to.
